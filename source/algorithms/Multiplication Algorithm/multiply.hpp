...
static std::string multiply_whole(std::string a, std::string b) {
    ...
    for (int i=0; i<b_vec.size(); i++) {
        ...
        for (int j=0; j<i; j++) to_push.push_back(0);
        for (int j=0; j<a_vec.size(); j++) to_push.push_back(b_vec[i] * a_vec[j]);
        to_add.push_back(to_push);
    }

    for (int i=0; i<to_add.size(); i++) { //Here, to_add[i] will be every element in to_add    //This enitre thing is to deal with carry forwards
        for (int j=0; j<to_add[i].size(); j++) { // to_add[i][j] will be every element in to_add[i]
            if (std::to_string(to_add[i][j]).length() > 1 && j != to_add[i].size()-1) {
    ...
    for (auto i : to_add) {
        std::string str = "";
        for (int j=i.size()-1; j>-1; j--) str += std::to_string(i[j]);
        str_to_add.push_back(str);
    }

    std::string answer = "0";
    for (auto i : str_to_add) answer = add(answer, i);
...
std::string multiply(std::string a, std::string b) {
    ...
    char sign = '+';
    if (a[0] == '-' && b[0] != '-') {sign = '-';a = a.substr(1, a.length());}
    if (b[0] == '-' && a[0] != '-') {sign = '-'; b = b.substr(1, b.length());}
    ...
    int n_to_shift = a.substr(a_dec_loc + 1, a.length()).length() + b.substr(b_dec_loc + 1, b.length()).length();
