...
std::pair<std::string, std::string> split_middle_term_ax2bxc(std::vector<algebric_num::algebric_number> terms) {
  if (terms.size() > 3)
    throw std::
  ...
  auto x = integer_square_root(add(multiply(terms[1].constantPart, terms[1].constantPart),multiply("-4",multiply(terms[0].constantPart, terms[2].constantPart))), 8);
