...
static bool str_contains_copy(const std::string &str, char charac) {
  for (auto &i : str)
    if (i == charac)
      return true;
  return false;
...
static std::string add_whole_copy(const std::string &a1,
                                  const std::string &b1) {
  std::string a = a1;
  std::string b = b1;
  if (a1 == "")
...
std::string add_copy(std::string a, std::string b) {
  if (!(str_contains_copy(a, '.') || str_contains_copy(b, '.'))) {
    ...
    a.erase(a_dec_loc, 1);
    b.erase(b_dec_loc, 1);
    return shift_decimal_point(add_whole_copy(a, b), -1 * n_to_shift);
  }
}
#endif
